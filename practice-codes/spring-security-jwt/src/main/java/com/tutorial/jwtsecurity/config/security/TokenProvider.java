...
import com.tutorial.jwtsecurity.domain.user.Authority;
    ...
    public Authentication getAuthentication(String refreshToken)throws TempJwtException { //JWT 토큰을 복호화하여 토큰에 들어 있는 정보를 꺼냅니다.
       ...
       //로그인 없이 재발급 했을때
        if (claims.get(AUTHORITIES_KEY) == null) {
            throw new TempJwtException(ErrorCode.NEED_AUTHORITIES.getMessage(), ErrorCode.NEED_AUTHORITIES);
        }
...
//    }
    ...
    public boolean validateToken(String token)throws TempJwtException {
        try {
            ...
            log.info("잘못된 JWT 서명입니다.");
            throw new TempJwtException(ErrorCode.WRONG_TYPE_TOKEN.getMessage(), ErrorCode.WRONG_TYPE_TOKEN);
        } catch (ExpiredJwtException e) {
            log.info("만료된 JWT 토큰입니다.");
